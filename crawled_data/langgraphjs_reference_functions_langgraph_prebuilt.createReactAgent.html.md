---
url: https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph_prebuilt.createReactAgent.html
title: Untitled
date_crawled: 
---
...
[](#)
  ...
  * [createReactAgent](langgraph_prebuilt.createReactAgent.html)



# Function createReactAgent

  * createReactAgent<[A](langgraph_prebuilt.createReactAgent.html#createReactAgent.A), [StructuredResponseFormat](langgraph_prebuilt.createReactAgent.html#createReactAgent.StructuredResponseFormat)>(params): [CompiledStateGraph](../classes/langgraph.CompiledStateGraph.html)<typeof [MessagesAnnotation](../variables/langgraph.MessagesAnnotation.html)["State"], typeof [MessagesAnnotation](../variables/langgraph.MessagesAnnotation.html)["Update"], any, typeof MessagesAnnotation.spec & [A](langgraph_prebuilt.createReactAgent.html#createReactAgent.A)["spec"], ReturnType<typeof createReactAgentAnnotation>["spec"] & [A](langgraph_prebuilt.createReactAgent.html#createReactAgent.A)["spec"]>[](#createReactAgent)
  * Creates a StateGraph agent that relies on a chat model utilizing tool calling.

#### Type Parameters

    * A extends [AnnotationRoot](../classes/langgraph.AnnotationRoot.html)<any, [A](langgraph_prebuilt.createReactAgent.html#createReactAgent.A)> = [AnnotationRoot](../classes/langgraph.AnnotationRoot.html)<{}>
    * StructuredResponseFormat extends Record<string, any> = Record<string, any>

#### Parameters

    * params: [CreateReactAgentParams](../types/langgraph_prebuilt.CreateReactAgentParams.html)<[A](langgraph_prebuilt.createReactAgent.html#createReactAgent.A), [StructuredResponseFormat](langgraph_prebuilt.createReactAgent.html#createReactAgent.StructuredResponseFormat)>

#### Returns [CompiledStateGraph](../classes/langgraph.CompiledStateGraph.html)<typeof [MessagesAnnotation](../variables/langgraph.MessagesAnnotation.html)["State"], typeof [MessagesAnnotation](../variables/langgraph.MessagesAnnotation.html)["Update"], any, typeof MessagesAnnotation.spec & [A](langgraph_prebuilt.createReactAgent.html#createReactAgent.A)["spec"], ReturnType<typeof createReactAgentAnnotation>["spec"] & [A](langgraph_prebuilt.createReactAgent.html#createReactAgent.A)["spec"]>

#### Example

```
import { ChatOpenAI } from"@langchain/openai";import { tool } from"@langchain/core/tools";import { z } from"zod";import { createReactAgent } from"@langchain/langgraph/prebuilt";constmodel = newChatOpenAI({model:"gpt-4o",});constgetWeather = tool((input) => {if (["sf", "san francisco"].includes(input.location.toLowerCase())) {return"It's 60 degrees and foggy."; } else {return"It's 90 degrees and sunny."; }}, {name:"get_weather",description:"Call to get the current weather.",schema:z.object({location:z.string().describe("Location to get the weather for."), })})constagent = createReactAgent({ llm:model, tools: [getWeather] });constinputs = {messages: [{ role:"user", content:"what is the weather in SF?" }],};conststream = awaitagent.stream(inputs, { streamMode:"values" });forawait (const { messages } ofstream) {console.log(messages);}// Returns the messages in the state at each step of execution
Copy
```


    * Defined in libs/langgraph/dist/prebuilt/react_agent_executor.d.ts:130
