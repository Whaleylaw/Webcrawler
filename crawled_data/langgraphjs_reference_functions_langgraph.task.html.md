---
url: https://langchain-ai.github.io/langgraphjs/reference/functions/langgraph.task.html
title: Untitled
date_crawled: 
---
...
[](#)
  ...
  * [task](langgraph.task.html)



# Function task

  * task<[ArgsT](langgraph.task.html#task.ArgsT), [OutputT](langgraph.task.html#task.OutputT)>(optionsOrName, func): ((...args) => Promise<[OutputT](langgraph.task.html#task.OutputT)>)[](#task)
  * `Beta`
...
Tasks can only be called from within an [entrypoint](langgraph.entrypoint.html) or from within a StateGraph. A task can be called like a regular function with the following differences:
...
#### Parameters
    ...
    * optionsOrName: string | [TaskOptions](../types/langgraph.TaskOptions.html)

Either an [TaskOptions](../types/langgraph.TaskOptions.html) object, or a string for the name of the task

    * func: TaskFunc<[ArgsT](langgraph.task.html#task.ArgsT), [OutputT](langgraph.task.html#task.OutputT)>

The function that executes this task

#### Returns ((...args) => Promise<[OutputT](langgraph.task.html#task.OutputT)>)

A proxy function that accepts the same arguments as the original and always returns the result as a Promise

    *       * (...args): Promise<[OutputT](langgraph.task.html#task.OutputT)>
      * #### Parameters

        * `Rest` ...args: [ArgsT](langgraph.task.html#task.ArgsT)

#### Returns Promise<[OutputT](langgraph.task.html#task.OutputT)>

#### Example: basic example

```
constaddOne = task("add", async (a: number) =>a + 1);constworkflow = entrypoint("example", async (numbers: number[]) => {constpromises = numbers.map(n=>addOne(n));constresults = awaitPromise.all(promises);returnresults;});// Call the entrypointawaitworkflow.invoke([1, 2, 3]); // Returns [2, 3, 4]
Copy
```
...
```
constaddOne = task({name:"add",retry: { maxAttempts:3 } },async (a: number) =>a + 1);constworkflow = entrypoint("example", async (numbers: number[]) => {constpromises = numbers.map(n=>addOne(n));constresults = awaitPromise.all(promises);returnresults;});
Copy
```


    * Defined in libs/langgraph/dist/func/index.d.ts:83
