...
[ Skip to content ](#testing-and-evals)
...
from fake_database import DatabaseConn [](#__code_0_annotation_1)
from weather_service import WeatherService [](#__code_0_annotation_2)
weather_agent = Agent(
  'openai:gpt-4o',
...
def weather_forecast(
  ...
  if forecast_date < date.today(): [](#__code_0_annotation_3)
    return ctx.deps.get_historic_weather(location, forecast_date)
  else:
    return ctx.deps.get_forecast(location, forecast_date)

async def run_weather_forecast( [](#__code_0_annotation_4)
...
pytestmark = pytest.mark.anyio [](#__code_1_annotation_1)
models.ALLOW_MODEL_REQUESTS = False [](#__code_1_annotation_2)

async def test_forecast():
  conn = DatabaseConn()
  user_id = 1
  with capture_run_messages() as messages:
    with weather_agent.override(model=TestModel()): [](#__code_1_annotation_3)
      prompt = 'What will the weather be like in London on 2024-11-28?'
      await run_weather_forecast([(prompt, user_id)], conn) [](#__code_1_annotation_4)
  forecast = await conn.get_forecast(user_id)
  assert forecast == '{"weather_forecast":"Sunny with a chance of rain"}' [](#__code_1_annotation_5)
  assert messages == [ [](#__code_1_annotation_6)
    ModelRequest(
      parts=[
        SystemPromptPart(
        ...
        UserPromptPart(
          ...
          timestamp=IsNow(tz=timezone.utc), [](#__code_1_annotation_7)
        ),
      ]
    ),
    ModelResponse(
      parts=[
        ToolCallPart(
          ...
          args={
            'location': 'a',
            'forecast_date': '2024-01-01', [](#__code_1_annotation_8)
          },
          tool_call_id=None,
        )
      ],
...
def call_weather_forecast( [](#__code_2_annotation_1)
  ...
  if len(messages) == 1:
    ...
    args = {'location': 'London', 'forecast_date': m.group()} [](#__code_2_annotation_2)
...
async def test_forecast_future():
  conn = DatabaseConn()
  user_id = 1
  with weather_agent.override(model=FunctionModel(call_weather_forecast)): [](#__code_2_annotation_3)
...
class SqlSystemPrompt: [](#__code_4_annotation_1)
  def __init__(
    ...
    self.db = db
  def build_prompt(self) -> str: [](#__code_4_annotation_2)
    return f"""\
...
"""
  @staticmethod
  def format_example(example: dict[str, str]) -> str: [](#__code_4_annotation_3)
    return f"""\
<example>
